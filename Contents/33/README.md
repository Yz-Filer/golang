...
# 33. （まとめ2）タブ切り替え型の簡易メモ帳  

![](image/window.jpg)  

タブ切り替え型の簡易メモ帳を作成していきます。  
上部ボタンは左から、「新規」「ファイルを開く」「上書き保存」「名前を付けて保存」「undo」「redo」「Wrap（折り返し）」となります。  
ステータスバーには、「カーソル位置」「改行コード」「文字コード」「スケール（拡大/縮小）」を表示します。  
タブは編集済みになると先頭に「*」を表示し、横幅は最大20文字、縦幅は最大2行にしています。  

## 33.1 GladeでUIを作成  

![](image/glade.jpg)  

メインウィンドウにはToolbar、Notebook、Statusbarを縦に並べています。  
ScrolledWindowコンテナは、Notebookのページになり、TextViewを配置しています。  
EventBoxは、Notebookのタブになり、LabelとButtonを配置しています。  

## 33.2 メインウィンドウ  

メインウィンドウには、「ボタン押下時の処理」「ページ変更時の処理」「スライダー変更時の処理」など、ページ内部の処理以外の処理を設定していきます。  
...
1. 新規ボタン押下時の処理  
   ...
   新規ボタン押下時は、ページの追加を行います。  
   「28章～32章」では外部スコープで定義した変数に代入していた物も、今回はページ毎に管理する必要があるため、`FileStr`構造体へ値を代入しています。  
   「開く」でも上記関数をコール出来るように、可変引数が指定された場合に、引数で渡されたデータを`FileStr`構造体へ代入しています。  
   `scrolledWindow1.SetName(id)`により、ScrolledWindowに名前を設定することで、ScrolledWindowがシグナルハンドラの引数で渡された時に、ページの判別が出来るように設定しています。  
   
   ラベルのプロパティ設定は以下のようなコードになります。  
   ...
   「最大文字数」をファイル名から計算しています。他の設定はコード内のコメントを読めば分かると思います。  

   「カーソル位置の表示」シグナルハンドラは、ステータスバーのLabelにカーソル位置を表示しています。  

   「ページCloseボタン押下時の処理」シグナルハンドラは、編集済みの場合に、ファイルを保存するかどうか確認し、保存する場合は「上書き保存」/「名前を付けて保存」をコールしてます。各保存関数については、「上書き保存」/「名前を付けて保存」ボタン押下時の処理の説明を参照してください。  

   その他の処理は、「28章～32章」とあまり変わらないので、説明は端折ります。  

1. 開くボタン押下時の処理  

   開くボタン押下時の処理は以下のようなコードになります。  
   ファイル選択ダイアログについては、
   ...
   「30章」で説明した文字コード判定を行っています。  
   改行コードは、「CRLF」「LF」のみを想定しており「CR」は想定してません。  

   読み込んだファイルは、UTF8へ文字コード変換を行います。utf8への文字コード変換処理は以下のようなコードになります。  
   ...
   文字コード変換処理が終わったら、最後に、「ページを追加」の関数をコールしています。  


1. 上書き保存ボタン押下時の処理  
   
   上書き保存ボタン押下時の処理は以下のようなコードになります。  
   ...
   改行コードをページの改行コードに統一した後、文字コード変換を行い、ファイルに保存します。保存した後は、未編集状態に各変数を設定しています。  
   文字コード変換処理は以下のようなコードになります。  
   ...
   utf8へ変換した時と逆にEncode処理となります。  
   
1. 名前を付けて保存ボタン押下時の処理  
   
   名前を付けて保存ボタン押下時の処理は以下のようなコードになります。  
   ファイル選択ダイアログについては、
   「[10. メニューバー/ツールバー/標準ダイアログを使いたい(後編)](../10/README.md)」
   を参照してください。  
   ...
   ファイルの保存は「上書き保存ボタン押下時の処理」で作成した関数で実施しているため、ファイル選択ダイアログの処理のみとなります。  
   「名前を付けて保存」の場合、別名保存が可能なように、未編集でも保存出来るようにしています。

1. undoボタン押下時の処理  

   「32章」で説明したのとあまり変わらないので省略します。  
...
1. wrapボタン押下時の処理  
   ...
   「選択ページ変更時の処理」で、wrapボタンの状態を変更しているため、「選択ページ変更時の処理」でシグナルが飛んできた場合はフラグをoffにして終了しています。  
   wrapのモード変更は`textView.SetWrapMode()`をコールしています。  

1. スケールの値が変更された時の処理  

   「31章」で説明したのとあまり変わらないので省略します。  

1. 選択ページ変更時の処理  
   
   選択ページ変更時の処理は以下のようなコードになります。  
   ...
   "switch-page"シグナルはページ変更の直前に発行されます。そのため、現在選択されてるページに対して変更処理を行うと、ページ変更前のページへ反映されてしまうので、ページ変更後のページに反映したい場合は、引数で受け取った変数を使う必要があります。  

   処理内容は、ページ毎の差異を反映しているだけなので、コードを見れば分かると思います。  

## 33.3 おわりに  

タブ切り替え型の簡易メモ帳を作成しました。  
「28章」で記載したように、「検索/置換」や「半角スペース/全角スペース/タブの視覚化」などを追加すれば、普通のエディタのように使えるようになるかと思います。  
また、改行時に改行前の行の先頭空白やタブをコピーして、改行後の先頭に貼り付けるようにするなど、細かい制御を入れると便利になるかもしれません。  
