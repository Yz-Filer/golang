...
メインウィンドウには、「ボタン押下時の処理」「ページ変更時の処理」「スケール変更時の処理」など、ページ内部の処理以外の処理を設定していきます。  
...
1. 新規ボタン押下時の処理  
   ...
   「28章～32章」では外部スコープで定義した変数に代入していた物もページ毎に管理する必要があるため、`FileStr`構造体へ値を代入しています。  
...
「28章」で記載したように、「検索/置換」や「半角スペース/全角スペース/タブの視覚化」など追加すれば、普通のエディタのように使えるようになると思います。  
また、改行時に改行前の行の先頭空白やタブをコピーして、改行後の先頭に貼り付けるようにするなど、細かい制御を入れると便利なるかもしれません。  
